  ...
  async (from, client, conText) => {
    ...
    try {
      ...
      if (q.match(/(youtube\.com|youtu\.be)/i)) {
        ...
        videoUrl = firstVideo.url || `https://www.youtube.com/watch?v=${firstVideo.id}`;
        videoId = firstVideo.id || extractVideoId(videoUrl);
        videoTitle = firstVideo.name || firstVideo.title || "YouTube Video";
        videoThumbnail = firstVideo.thumbnail || firstVideo.image || `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
        videoDuration = firstVideo.duration || firstVideo.timestamp || "Unknown";
        videoViews = firstVideo.views || "Unknown";
        videoChannel = firstVideo.author?.name || firstVideo.channel || "Unknown";
      }
      ...
      const infoBoxContext = {
        ...
        externalAdReply: {
          title: videoTitle,
          body: "CORAZONE 002 | Action over Talks!!",
          mediaType: 2,
      ...
      const handleReply = async (update) => {
        ...
        try {
          ...
          const fetchMedia = async (apiCall, retries = 2) => {
            for (let i = 0; i <= retries; i++) {
              try {
                ...
                const result = response.data?.result || response.data?.download_url || response.data?.url;
          ...
          switch (selectedIndex) {
            case 1:
              try {
                const audioUrl = await fetchMedia(XMD.API.DOWNLOAD.YOUTUBE_AUDIO(videoUrl));
                ...
                await client.sendMessage(destChat, { react: { text: "❌", key: message.key } });
                await client.sendMessage(destChat, { text: "❌ Audio failed. The API might be down." }, { quoted: message });
              }
              break;

            case 2:
              try {
                const videoDownloadUrl = await fetchMedia(XMD.API.DOWNLOAD.YOUTUBE(videoUrl));
                if (!videoDownloadUrl) throw new Error("Video not available");
                await client.sendMessage(destChat, {
                ...
                await client.sendMessage(destChat, { react: { text: "❌", key: message.key } });
                await client.sendMessage(destChat, { text: "❌ Video failed. The API might be down." }, { quoted: message });
              }
              break;
